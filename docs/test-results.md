...
## 七、回归评测（2026-09-18，v1.0.0 结构治理与资源扩充后）

评测对象：新增的 `references/workflow.md` 全流程编排与 `resources/` 下 19 个模板。完整报告见 `supplement-audits/v1.0.0-workflow-templates-audit.md`。

| 项 | 结果 |
|---|---|
| 路径引用机械检查 | 修复前 20 条打不开（14 条从任何基准都解析不了）→ 修复后 0 条；`validate.py` 增加裸路径检查防复发 |
| E1 从零跑完整项目 | 30/40 → **38/40** PASS（修复：适用性确认、全步骤输入行、参考工期、第 5 步模板） |
| E2 要求跳过闸门直接铺量 | **37/40** PASS（三条拦截点齐备，未发现缺陷） |
| E3 稿件体检（1200 字头条短文） | 31/40 → **38/40** PASS（修复：结构硬指标误判平台短文、两套百分制并列无说明） |
| E4 robots + llms.txt + schema | 38/40 → **39/40** PASS（修复：schema 模板预填像真数据的评分，与"不伪造评价"红线冲突） |
| 仓库一致性校验 | `python scripts/validate.py` 0 errors 0 warnings |

共发现并修复 11 项缺陷，其中 5 项为高严重度（会导致实际答错或触碰红线）。

**局限声明**：本轮场景评测由撰写者自评，非独立 sub-agent 盲测——"覆盖度"与"利用率"两维读数应打折；"准确度"维的扣分依据是可复核的内部矛盾（体检表与 one-fish 规范冲突、schema 模板与红线冲突），可信度较高。建议后续用干净 sub-agent 复跑 E1 与 E3。
