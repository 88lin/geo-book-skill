...
- v0.3.0 合入 [GeoLook](https://github.com/aigclink/geolook)（MIT）CN-GEO 数据集 v2.0.1（187,818 条去重引用实算）的大样本结论：官网 1.37% 定律、各引擎生态集中度、Web≠App 口径（千问两端信源重合 24.5%）、四层依赖模型、归因三段链路——卡内逐条标注来源；第三方数据集口径，与原书 12 题实测互相印证但不互相替代。
- **配套技能**：[GeoLook](https://github.com/aigclink/geolook) 本身就是一个独立 Skill（自托管全流程 GEO 实施平台：站点六维体检、17 引擎采样、工单与自动验收、客户交付包）。分工——geo-playbook 出方法论与决策（为什么、怎么选、怎么写），GeoLook 跑自动化流水线（诊断、采样、验收、交付）。两者可同时安装，触发场景基本不重叠。
...
首版：触发盲测（独立 agent、含跨 Skill 诱饵）**31/31**、路由可达 5/5、输出评测（12 能力 × 正常+边界）**24/24**。v0.2.0 回归：触发盲测 **15/15**、输出评测 **6/6**（含拒绝"AI 爬虫全部 Disallow"反模式）。v0.3.0 回归：输出评测 **2/2**（1.37% 数据反驳"官网中心"预算、归因三段算账），触发面未变沿用 15/15，边界场景全部正确"先询问/停下/拒绝"。详见 [`docs/test-results.md`](docs/test-results.md)。
